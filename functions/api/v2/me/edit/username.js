...
export async function onRequestPost(ctx) {
    try {
        ...
        if (await ctx.env.UsernameChangeTimelimit.get(ctx.data.userid)) {return error(400, `ERR_CANNOT_YET_CHANGE`)}
        ...
        await ctx.env.UsernameChangeTimelimit.put(ctx.data.userid, [('0' + formattedDate.getDate()).slice(-2), ('0' + (formattedDate.getMonth() + 1)).slice(-2), formattedDate.getFullYear().toString().slice(-2)].join('/'), {expirationTtl: monthFromNow})
        return status(204)
    } catch (e) {
        console.error(e)
        return error(e)
