...
import {Mails, Phone, TabletSmartphone, ArchiveRestore, Check, MailCheck, MailX} from 'lucide-vue-next';
...
<template>
  <Transition name="fade" mode="out-in">
    <div v-if="emailActive" class="space-y-3">
      <ConnectorAlert custom-title="You Cannot Disable Email as a MFA Method" custom-message="Every MXS account must have a valid email address connected and as such must always be available as a MFA method. You may edit your email address below:" />
      <EditDetailDialog
          title="Email Address"
    ...
    <div v-else-if="phoneActive" class="space-y-3 flex flex-col items-center">
      <p class="text-sm w-2/3 text-center">When a phone number is added to a MXS Account it is used <strong>only</strong> for MFA. To {{ userData.phone_number ? 'disable' : 'enable' }} MFA via SMS {{ userData.phone_number ? 'remove' : 'add' }} your number below:</p>
      <div class="w-2/3">
      <EditDetailDialog
          title="Phone Number"
