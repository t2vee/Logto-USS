...
async function removeNumber() {
  ...
  try {
    const response = await axios.post(`${import.meta.env.VITE_API_WORKER_ENDPOINT}/api/v1/user-data-entry/remove-verify-method/remove-sms?user-id=${userData.value.sub}`,
        {},
        {
          headers: {
    ...
    if (response.status === 204) {
      toast.success('Number Successfully Removed',{description: 'You can now add a new number to your account.'})
    }
  } catch (error) {
