...
<template>
  <Card class="w-full h-[30px] rounded-t-lg rounded-b-none flex items-center align-middle justify-evenly" :class="connector.classes">
    <component :is="connector.iconComponent" :fill="connector.primaryColour" />
    <p class="text-xs">You are connected using {{ connector.name }}. Some user details are omitted and cannot be changed.</p>
  </Card>
</template>
