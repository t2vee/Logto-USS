...
async function loadData() {
  ...
  try {
    ...
    const response = await axios.get(`${import.meta.env.VITE_API_WORKER_ENDPOINT}/api/v1/me/extended-user-info`, {
      headers: {
...
<template>
  <div class="flex flex-col items-center">
    <div class="flex flex-col h-screen max-w-[1000px] items-center">
      ...
      <div v-else-if="fetchFailure && !isLoading" class="flex flex-col items-center justify-center gap-y-3">
        <AlertOctagon :size="72" color="darkred" />
        <p class="text-red-700 text-xl">Failed to grab User information</p>
        <Button @click="loadData" variant="outline" class="h-8 w-20">
          Retry
        </Button>
      </div>
