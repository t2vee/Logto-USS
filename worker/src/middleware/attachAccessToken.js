...
export default async (req, env, ctx) => {
	let accessToken = await env.LogtoAccessToken_DEMO.get("access_token");
	let expiry = await env.LogtoAccessToken_DEMO.get("token_expiry");
	...
	if (!accessToken || !expiry || parseInt(expiry) <= now) {
		...
		try {
			...
			await env.LogtoAccessToken_DEMO.put("access_token", accessTokenResponse.access_token, { expirationTtl: accessTokenResponse.expires_in });
			await env.LogtoAccessToken_DEMO.put("token_expiry", (now + accessTokenResponse.expires_in).toString(), { expirationTtl: accessTokenResponse.expires_in });
